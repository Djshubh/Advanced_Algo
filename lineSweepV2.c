...
struct AVLNode* predlocal(struct AVLNode *node){
      //  struct AVLNode *node = localroot;
...
struct AVLNode* succlocal(struct AVLNode *node){
     //struct AVLNode *node = localroot;
...
struct AVLNode* pred(struct AVLNode *root,struct AVLNode *node){
       struct AVLNode *curr=root,*pr = predlocal(node);
       int findkey = node->key;
      // printf("pre of :%d //",node->key);
       if(root==node)
        return pr;
       if(pr==NULL){
           while(root->key!=findkey)
           {
             //  printf("%d ",root->key);
               curr=root;
               if(root->key<findkey)
           ...
           //  printf("%d ",curr->key);
         if(curr->right!=NULL && curr->right->key==findkey)
            return curr;
         else
            return NULL;
       }
...
struct AVLNode* succ(struct AVLNode *root,struct AVLNode *node){
       struct AVLNode *curr=root,*suc = succlocal(node);
      // printf("suc of :%d ",node->key);
       int findkey = node->key;
       if(root==node)
        return suc;
       if(suc==NULL){
         ...
         //printf("%d",curr->key);
         if(curr->left!=NULL && curr->left->key==findkey)
            return curr;
         else
            return NULL;
       }
...
int main()
{
  struct AVLNode *root = NULL,*newNode;
  int size = 9,i,count;
  ...
  for(i=0;i<=size*2-1;i++){
    ...
    if(isleft(i))
    {
        ...
        if(intersect(newNode,succ(root,newNode)))
        {
            k = succ(root,newNode);
            printf(" lines segments ::%d %d\n",newNode->start.ptr,k->start.ptr);
           //printf("%SUXX");
        }

        //check intersection with predecessor
        if(intersect(newNode,pred(root,newNode)))
         {
             k = pred(root,newNode);
            printf(" lines segments ::%d %d\n",newNode->start.ptr,k->start.ptr);
    ...
    {
        printf("\n:rightloc %d %d:\n",scat[i].xc,scat[i].yc);
        if(intersect(pred(root,newNode),succ(root,newNode)))
         {
              k = pred(root,newNode);
             struct AVLNode *q =succ(root,newNode);
            printf(" lines segments ::%d %d\n",q->start.ptr,k->start.ptr);
